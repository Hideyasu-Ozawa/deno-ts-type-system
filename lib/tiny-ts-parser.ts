...
export type TypeForArith = { tag: "Boolean" } | { tag: "Number" };
...
function convertType(node: p.TSESTree.TypeNode): Type {
  switch (node.type) {
    ...
    case "TSFunctionType": {
      ...
      const typeParams = node.typeParameters.params.map(
        (typeParameter) => typeParameter.name.name
      );
...
function newIfNode(cond: Term, thn: Term, els: Term, loc: Location): Term {
  if (cond.tag === "compare") {
    if (
      cond.left.tag === "objectGet" &&
      cond.left.obj.tag === "var" &&
...
type ExtractTagsSub<T, K> = T extends Type | Term
  ? ExtractTags<T, K>
  : T extends Type[]
  ? ExtractTags<Type, K>[]
  : T extends Term[]
  ? ExtractTags<Term, K>[]
  : T extends (infer U)[]
  ? { [P in keyof U]: ExtractTags<U[P], K> }[]
  : T;
